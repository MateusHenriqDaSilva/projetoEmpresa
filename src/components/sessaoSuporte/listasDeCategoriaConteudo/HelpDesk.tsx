...
export default function HelpDesk({ itemsPorPagina }) {
    const listaDeHelpDesk: any[] = []
    ...
    dadosCartao.map((elemento) => {
        elemento.categoria.map((categoria) => {
            if (categoria == 'helpdesk') {
                listaDeHelpDesk.push(elemento)
            }
        })
    })
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeHelpDesk.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeHelpDesk.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeHelpDesk.length;
        console.log(
