...
export default function Transportadora({ itemsPorPagina }) {
    const listaDeTransportadora: any[] = []
    ...
    dadosCartao.map((elemento) => {
        elemento.categoria.map((categoria) => {
            if (categoria == 'transportadora') {
                listaDeTransportadora.push(elemento)
            }
        })
    })
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeTransportadora.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeTransportadora.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeTransportadora.length;
        console.log(
