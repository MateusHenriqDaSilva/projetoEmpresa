...
export default function Etiquetas({ itemsPorPagina }) {
    const listaDeEtiquetas: any[] = []
    ...
    dadosCartao.map((elemento) => {
        elemento.categoria.map((categoria) => {
            if (categoria == 'etiquetas') {
                listaDeEtiquetas.push(elemento)
            }
        })
    })
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeEtiquetas.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeEtiquetas.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeEtiquetas.length;
        console.log(
