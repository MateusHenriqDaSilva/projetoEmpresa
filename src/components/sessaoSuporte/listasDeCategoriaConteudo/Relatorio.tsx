...
export default function Relatorio({ itemsPorPagina }) {
    const listaDeRelatorio: any[] = []
    ...
    dadosCartao.map((elemento) => {
        elemento.categoria.map((categoria) => {
            if (categoria == 'relatorio') {
                listaDeRelatorio.push(elemento)
            }
        })
    })
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeRelatorio.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeRelatorio.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeRelatorio.length;
        console.log(
