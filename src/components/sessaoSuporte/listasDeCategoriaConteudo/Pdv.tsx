...
export default function Pdv({ itemsPorPagina }) {
    const listaDePdv: any[] = []
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDePdv.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDePdv.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDePdv.length;
        console.log(
