...
export default function Financeiro({ itemsPorPagina }) {
    const listaDeFinanceiro: any[] = []
    ...
    dadosCartao.map((elemento) => {
        elemento.categoria.map((categoria) => {
            if (categoria == 'financeiro') {
                listaDeFinanceiro.push(elemento)
            }
        })
    })
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeFinanceiro.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeFinanceiro.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeFinanceiro.length;
        console.log(
