...
export default function Estoque({ itemsPorPagina }) {
    const listaDeEstoque: any[] = []
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeEstoque.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeEstoque.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeEstoque.length;
        console.log(
