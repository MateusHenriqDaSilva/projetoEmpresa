...
export default function Administrador({ itemsPorPagina }) {
    const listaDeAdministrador: any[] = []
    ...
    dadosCartao.map((elemento) => {
        elemento.categoria.map((categoria) => {
            if (categoria == 'administrador') {
                listaDeAdministrador.push(elemento)
            }
        })
    })
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeAdministrador.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeAdministrador.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeAdministrador.length;
        console.log(
