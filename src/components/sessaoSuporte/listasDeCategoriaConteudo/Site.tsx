...
export default function Site({ itemsPorPagina }) {
    const listaDeSite: any[] = []
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeSite.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeSite.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeSite.length;
        console.log(
