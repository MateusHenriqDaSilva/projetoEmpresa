...
export default function Kit({ itemsPorPagina }) {
    const listaDeKit: any[] = []
    ...
    useEffect(() => {
        ...
        setCurrentItems(listaDeKit.slice(itemOffset, endOffset));
        setPaginasContadas(Math.ceil(listaDeKit.length / itemsPorPagina));
    }, [itemOffset, itemsPorPagina]);

    const paginasPorClick = (evento) => {
        const newOffset = (evento.selected * itemsPorPagina) % listaDeKit.length;
        console.log(
